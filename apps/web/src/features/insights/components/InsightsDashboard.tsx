...
import { insightsApi, InsightsIncident, InsightsResponseTime, InsightsWorkload, InsightsPeakTime, IncidentCategory } from "../api/insights.api";
import { InsightsFilterBar } from "./InsightsFilterBar";
import { ResponseTimeStats } from "./ResponseTimeStats";
import { WorkloadPanel } from "./WorkloadPanel";
import { PeakTimePatterns } from "./PeakTimePatterns";
import { useSocket } from "@/lib/socket";
import { FiEye as Eye, FiEyeOff as EyeOff, FiBell as Bell, FiMaximize2 as Maximize, FiMinimize2 as Minimize } from "react-icons/fi";

const IncidentMapView = dynamic(() => import("./IncidentMapView").then(mod => mod.IncidentMapView), { 
  ssr: false, 
  loading: () => <div className="animate-pulse bg-gray-100 rounded-lg h-full min-h-[400px]"></div> 
});
  ...
  const panels: PanelConfig[] = [
    {
      ...
      content: <IncidentMapView incidents={incidents} isLoading={loadingIncidents} selectedBarangay={filters.barangay} />
    },
    {
      id: "responseTimes",
      title: "Response Times",
      colSpan: "xl:col-span-1",
      content: <ResponseTimeStats data={responseTimes} isLoading={loadingAggregates} />
    },
    {
      id: "peakTimes",
      title: "Peak Time Patterns",
      colSpan: "xl:col-span-1",
      content: <PeakTimePatterns data={peakTimes} isLoading={loadingAggregates} />
    },
    {
      id: "workload",
  ...
  return (
    <div className="flex flex-col h-[calc(100vh-80px)] overflow-hidden gap-5">
      <div className="flex items-center justify-between shrink-0">
        <div>
          <h1 className="display-small text-foreground">Operational Insights</h1>
      ...
      <div className="flex flex-1 gap-5 min-h-0 overflow-hidden">
        ...
        {collapsedPanels.length > 0 && (
          <div className="w-16 md:w-64 shrink-0 bg-white border border-gray-100 rounded-lg p-2 md:p-4 flex flex-col gap-3 overflow-y-auto shadow-sm">
            <h4 className="text-xs font-bold text-gray-400 uppercase tracking-wider mb-1 hidden md:block px-1">Collapsed Dock</h4>
              ...
              return (
                ...
                >
                  ...
                  <div className="relative">
                    <Eye size={18} className={`${hasUnread ? 'text-danger' : 'text-gray-400 group-hover:text-primary'}`} />
                    {hasUnread && (
