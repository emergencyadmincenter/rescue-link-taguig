...
import { FiX, FiMic, FiMicOff, FiVideo, FiVideoOff, FiMessageSquare, FiCheck } from 'react-icons/fi';
...
export default function ActiveSOSView({ callId, callData, socket }: ActiveSOSViewProps) {
  const [isMuted, setIsMuted] = React.useState(false);
  const [isVideoOn, setIsVideoOn] = React.useState(false);
  const [isChatOpen, setIsChatOpen] = React.useState(false);
  const [sessionEndReason, setSessionEndReason] = React.useState<'resident' | 'coordinator' | 'system' | null>(null);
  
  const audioRef = useRef<HTMLAudioElement>(null);

  const { startCall, endCall, toggleMute, remoteStream } = useWebRTC(socket, callId, 'resident');
  ...
  if (sessionEndReason) {
    return (
      <div className="min-h-screen bg-gray-50 flex flex-col items-center justify-center p-6 animate-in fade-in">
        <div className="bg-white rounded-3xl shadow-lg border border-gray-100 p-10 max-w-md w-full flex flex-col items-center text-center">
          ...
          <button 
            ...
            className="w-full py-3 bg-gray-900 text-white rounded-xl font-medium hover:bg-black transition-colors mb-3"
          >
            Go Back Home
          </button>
        </div>
    ...
    <div className="min-h-screen bg-black flex flex-col animate-in fade-in">
      ...
      <div className="p-4 flex justify-between items-center bg-gradient-to-b from-black/80 to-transparent absolute top-0 w-full z-10">
        <div className="flex items-center gap-2">
          ...
          <span className="text-white text-sm font-medium">Active Emergency Call</span>
        </div>
        <span className="text-white/60 text-sm font-mono">{callId.slice(0,8)}</span>
      </div>
        ...
        {isChatOpen && (
          <div className="absolute inset-x-4 bottom-32 top-20 bg-white/10 backdrop-blur-md rounded-2xl border border-white/20 flex flex-col overflow-hidden">
            <div className="p-3 border-b border-white/20 flex justify-between items-center bg-black/40">
              <span className="text-white font-medium text-sm">Emergency Chat</span>
      ...
      <div className="p-6 pb-8 flex justify-center items-center gap-6 bg-gradient-to-t from-black/80 to-transparent absolute bottom-0 w-full">
        ...
        <button 
          ...
          className={`p-4 rounded-full transition-colors ${isVideoOn ? 'bg-white text-black' : 'bg-white/10 text-white hover:bg-white/20'}`}
        >
        ...
        >
          {!isMuted ? <FiMic className="w-6 h-6" /> : <FiMicOff className="w-6 h-6" />}
        </button>

        <button 
          onClick={handleEndCall}
          className="p-4 rounded-full bg-danger text-white hover:bg-danger-hover transition-colors"
        >
