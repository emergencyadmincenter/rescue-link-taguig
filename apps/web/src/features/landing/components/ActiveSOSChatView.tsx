...
export default function ActiveSOSChatView({ callId, callData, socket }: ActiveSOSChatViewProps) {
  const [messages, setMessages] = useState<any[]>([]);
  const [text, setText] = useState('');
  const [sessionEndReason, setSessionEndReason] = useState<'resident' | 'coordinator' | 'system' | null>(null);
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="flex flex-col">
            ...
            <div className="flex items-center gap-1.5">
              ...
              <span className="text-success text-xs font-medium">Active Chat</span>
            </div>
          </div>
        </div>
        <button 
